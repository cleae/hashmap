...
public class Tree {
    ...
    public static int shareCake(int n){
        int[] arr = new int[n];
        ...
        while(len>1){
            int nextLen = len/2;
            if (len % 2 == 0 && !leftFlag){//从左边第2个开始
                for (int i = 1,j = 2; i < nextLen; i ++,j+=2){
                    arr[i] = arr[j];
                }
            }else{//从第1个开始
                for (int i = 0,j = 1; i < nextLen; i ++,j+=2){
                    arr[i] = arr[j];
                }
            }
            len = nextLen;
...
//        Integer[] lever={3,9,20,null,null,15,7};
        ...
        System.out.println(shareCake(6));
    }
